...
SELECT DISTINCT
	WsTab.lt_table_name,
	CONVERT(VARCHAR(MAX),WsTab.lt_source_table),
	COALESCE(WsObj.TargetName,'TGT_PH'), 
	REPLACE('FROM {{ ref(''' + COALESCE(WsObj.TargetName,'TGT_PH') + ''',''' + CONVERT(VARCHAR(MAX),WsTab.lt_source_table) + ''') }}','~',' ') + ' ' + CONVERT(VARCHAR(MAX),lt_where_clause),
	WsTab.lt_table_name
...
LEFT JOIN #WsToClRepoSchema.ObjectDetails WsObj
  ON CONVERT(VARCHAR(MAX),WsTab.lt_source_table) = WsObj.ObjectName
