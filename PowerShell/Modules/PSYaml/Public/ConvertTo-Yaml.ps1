        ...
        try
        {
            ...
            $output += switch ($Type)
            {
                ...
                'Array'    { "$($inputObject | Foreach-Object { 
                        ...
                        "`r`n$padding  - $((ConvertTo-YAML -inputObject $_ -depth $depth -NestingLevel ($NestingLevel + 1)).Trim())" })" 
                 }
                'HashTable'{
                    ("$($inputObject.GetEnumerator() | Foreach-Object {
                      if ($_.Value.GetType().Name -eq 'String'){$increment=2} else {$Increment=1}
                      if($_.Value -eq '' -or $_.Value.Count -eq 0)
                      {
                       
                        if($_.Value.GetType().Name -eq 'Object[]')
                        {
                          if($_.Name -eq "columnReferences")
                          {                            
                            "`r`n$padding  - $($_.Name): []"
                        ...
                        elseif($_.Value.GetType().Name -eq 'OrderedDictionary')
                        {
                          if($_.Name -eq "aliases")
                          {                           
                        ...
                        elseif($_.Name -eq "transform" -and $isTransformNext -eq $true)
                        {
                          "`r`n$padding       $($_.Name): """"""""" 
                          $isTransformNext = $false
                        }                       
